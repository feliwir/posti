...
// [Guard]
#ifndef _B2D_PIPE2D_PIPECOMPILER_P_H
#define _B2D_PIPE2D_PIPECOMPILER_P_H
...
// ============================================================================
// [b2d::Pipe2D::PipeCompiler]
// ============================================================================

//! Pipeline compiler.
class PipeCompiler {
public:
  B2D_NONCOPYABLE(PipeCompiler)
  ...
  enum {
    ...
    kReservedGpRegs = 1 + uint32_t(B2D_ARCH_BITS >= 64),
  ...
  PipeCompiler(x86::Compiler* cc) noexcept;
  ...
  inline bool hasSSE2() const noexcept { return _optLevel >= kPipeOpt_X86_SSE2; }
  inline bool hasSSE3() const noexcept { return _optLevel >= kPipeOpt_X86_SSE3; }
  inline bool hasSSSE3() const noexcept { return _optLevel >= kPipeOpt_X86_SSSE3; }
  inline bool hasSSE4_1() const noexcept { return _optLevel >= kPipeOpt_X86_SSE4_1; }
  inline bool hasSSE4_2() const noexcept { return _optLevel >= kPipeOpt_X86_SSE4_2; }
  inline bool hasAVX() const noexcept { return _optLevel >= kPipeOpt_X86_AVX; }
  inline bool hasAVX2() const noexcept { return _optLevel >= kPipeOpt_X86_AVX2; }
  ...
  // TODO: There should be a getter on asmjit side that will return the `Zone`
  // object that can be used for these kind of purposes. It doesn't make sense
  // to create another Zone.
  template<typename T>
  inline T* newPartT() noexcept {
    return new(cc->_codeZone.alloc(sizeof(T))) T(this);
  }

  template<typename T, typename P0>
  inline T* newPartT(P0 p0) noexcept {
    return new(cc->_codeZone.alloc(sizeof(T))) T(this, p0);
  }

  template<typename T, typename P0, typename P1>
  inline T* newPartT(P0 p0, P1 p1) noexcept {
    return new(cc->_codeZone.alloc(sizeof(T))) T(this, p0, p1);
  }

  template<typename T, typename P0, typename P1, typename P2>
  inline T* newPartT(P0 p0, P1 p1, P2 p2) noexcept {
    return new(cc->_codeZone.alloc(sizeof(T))) T(this, p0, p1, p2);
  }

  template<typename T, typename P0, typename P1, typename P2, typename P3>
  inline T* newPartT(P0 p0, P1 p1, P2 p2, P3 p3) noexcept {
    return new(cc->_codeZone.alloc(sizeof(T))) T(this, p0, p1, p2, p3);
  }

  FillPart* newFillPart(uint32_t fillType, FetchPart* dstPart, CompOpPart* compOpPart) noexcept;
  FetchPart* newFetchPart(uint32_t fetchType, uint32_t fetchExtra, uint32_t pixelFormat) noexcept;
  ...
  void _initConstantsPtr() noexcept;
  ...
  B2D_NOINLINE void newXmmArray(OpArray& dst, uint32_t n, const char* name) noexcept {
    B2D_ASSERT(n <= OpArray::kMaxSize);
  ...
  B2D_NOINLINE void uZeroIfEq(const A& a, const B& b) noexcept {
    Label L = cc->newLabel();
  ...
  // dst = abs(src)
  template<typename DstT, typename SrcT>
  B2D_NOINLINE void uAbs(const DstT& dst, const SrcT& src) noexcept {
    if (dst.id() == src.id()) {
  ...
  B2D_NOINLINE void uBound0ToN(const DstT& dst, const ValueT& value, const LimitT& limit) noexcept {
    if (dst.id() == value.id()) {
  ...
  template<typename DstT, typename SrcT>
  B2D_NOINLINE void uReflect(const DstT& dst, const SrcT& src) noexcept {
    B2D_ASSERT(dst.size() == src.size());
  ...
  template<typename DstT, typename SrcT>
  B2D_NOINLINE void uMod(const DstT& dst, const SrcT& src) noexcept {
  ...
  B2D_NOINLINE void uAdvanceAndDecrement(const x86::Gp& p, int pAdd, const x86::Gp& i, int iDec) noexcept {
    cc->add(p, pAdd);
    cc->sub(i, iDec);
  }

  //! dst = a * b.
  B2D_NOINLINE void uMulImm(const x86::Gp& dst, const x86::Gp& a, int b) noexcept {
    if (b > 0) {
      switch (b) {
        case 1:
          if (dst.id() != a.id())
  ...
  //! dst += a * b.
  B2D_NOINLINE void uAddMulImm(const x86::Gp& dst, const x86::Gp& a, int b) noexcept {
    switch (b) {
      case 1:
        cc->add(dst, a);
        return;
  ...
  B2D_NOINLINE void uLeaBpp(const x86::Gp& dst, const x86::Gp& src, const x86::Gp& idx, uint32_t scale, int32_t disp = 0) noexcept {
    switch (scale) {
      case 1:
  ...
  V_EMIT_VVV_VV(vnand        , PACK_AVX_SSE(Vpandn     , Pandn     , Z))       // AVX2 | SSE2
  ...
  V_EMIT_VVV_VV(vnandps      , PACK_AVX_SSE(Vandnps    , Andnps    , Z))       // AVX  | SSE
  V_EMIT_VVV_VV(vnandpd      , PACK_AVX_SSE(Vandnpd    , Andnpd    , Z))       // AVX  | SSE2
  ...
  template<typename DstT, typename SrcT>
  B2D_NOINLINE void vduplpd(const DstT& dst, const SrcT& src) noexcept {
    vemit_vv_vv(PackedInst::packIntrin(kIntrin2Vduplpd), dst, src);
  }

  template<typename DstT, typename SrcT>
  B2D_NOINLINE void vduphpd(const DstT& dst, const SrcT& src) noexcept {
  ...
  // TODO: Consolidate this to only one implementation.
  template<typename DstSrcT>
  B2D_NOINLINE void vdiv255u16(const DstSrcT& x) {
    vaddi16(x, x, constAsXmm(gConstants.xmm_u16_0080));
    vmulhu16(x, x, constAsXmm(gConstants.xmm_u16_0101));
  }

  template<typename DstSrcT>
  B2D_NOINLINE void vdiv255u16_2x(
    const DstSrcT& v0,
    const DstSrcT& v1) noexcept {

    x86::Xmm xmm_u16_0080 = constAsXmm(gConstants.xmm_u16_0080);
    x86::Xmm xmm_u16_0101 = constAsXmm(gConstants.xmm_u16_0101);

    vaddi16(v0, v0, xmm_u16_0080);
    vmulhu16(v0, v0, xmm_u16_0101);

    vaddi16(v1, v1, xmm_u16_0080);
    vmulhu16(v1, v1, xmm_u16_0101);
  }

  template<typename DstSrcT>
  B2D_NOINLINE void vdiv255u16_3x(
    const DstSrcT& v0,
    const DstSrcT& v1,
    ...
    vaddi16(v2, v2, xmm_u16_0080);
    vmulhu16(v2, v2, xmm_u16_0101);
  }
  ...
  B2D_NOINLINE void xMovzxBW_LoHi(const x86::Vec& d0, const x86::Vec& d1, const x86::Vec& s) noexcept {
    B2D_ASSERT(d0.id() != d1.id());
    ...
    if (hasSSE4_1()) {
      ...
      x86::Xmm xmm_zeros = constAsXmm(gConstants.xmm_zeros);
      if (d1.id() != s.id()) {
        vunpackhi8(d1, s, xmm_zeros);
        vunpackli8(d0, s, xmm_zeros);
      }
      else {
        vunpackli8(d0, s, xmm_zeros);
        vunpackhi8(d1, s, xmm_zeros);
  ...
  template<typename DstT, typename SrcT>
  inline void vFillAlpha255B(const DstT& dst, const SrcT& src) noexcept { vor(dst, src, constAsMem(gConstants.xmm_u32_FF000000)); }
  template<typename DstT, typename SrcT>
  inline void vFillAlpha255W(const DstT& dst, const SrcT& src) noexcept { vor(dst, src, constAsMem(gConstants.xmm_u64_00FF000000000000)); }
  template<typename DstT, typename SrcT>
  inline void vFillAlpha256W(const DstT& dst, const SrcT& src) noexcept { vor(dst, src, constAsMem(gConstants.xmm_u64_0100000000000000)); }

  template<typename DstT, typename SrcT>
  inline void vZeroAlphaB(const DstT& dst, const SrcT& src) noexcept { vand(dst, src, constAsMem(gConstants.xmm_u32_00FFFFFF)); }
  template<typename DstT, typename SrcT>
  inline void vZeroAlphaW(const DstT& dst, const SrcT& src) noexcept { vand(dst, src, constAsMem(gConstants.xmm_u64_0000FFFFFFFFFFFF)); }

  template<typename DstT, typename SrcT>
  inline void vNegAlpha8B(const DstT& dst, const SrcT& src) noexcept { vxor(dst, src, constAsMem(gConstants.xmm_u32_FF000000)); }
  template<typename DstT, typename SrcT>
  inline void vNegAlpha8W(const DstT& dst, const SrcT& src) noexcept { vxor(dst, src, constAsMem(gConstants.xmm_u64_00FF000000000000)); }

  template<typename DstT, typename SrcT>
  inline void vNegRgb8B(const DstT& dst, const SrcT& src) noexcept { vxor(dst, src, constAsMem(gConstants.xmm_u32_00FFFFFF)); }
  template<typename DstT, typename SrcT>
  inline void vNegRgb8W(const DstT& dst, const SrcT& src) noexcept { vxor(dst, src, constAsMem(gConstants.xmm_u64_000000FF00FF00FF)); }

  // d = int(floor(a / b) * b).
  template<typename XmmOrMem>
  B2D_NOINLINE void vmodpd(const x86::Xmm& d, const x86::Xmm& a, const XmmOrMem& b) noexcept {
    if (hasSSE4_1()) {
      vdivpd(d, a, b);
      ...
      vandpd(d, d, constAsMem(gConstants.xmm_d_m1));
      vaddpd(d, d, t);
      vmulpd(d, d, b);
    }
  }
  ...
  B2D_NOINLINE void xModI64HIxU64LO(const x86::Xmm& d, const XmmOrMem_A& a, const XmmOrMem_B& b) noexcept {
  ...
  template<typename XmmOrMem_A, typename XmmOrMem_B>
  B2D_NOINLINE void xModI64HIxDouble(const x86::Xmm& d, const XmmOrMem_A& a, const XmmOrMem_B& b) noexcept {
    x86::Xmm t0 = cc->newXmm("t0");
  ...
  B2D_NOINLINE void xExtractUnpackedAFromPackedARGB32_1(const x86::Xmm& d, const x86::Xmm& s) noexcept {
    vswizli16(d, s, x86::Predicate::shuf(1, 1, 1, 1));
    vsrli16(d, d, 8);
  }

  B2D_NOINLINE void xExtractUnpackedAFromPackedARGB32_2(const x86::Xmm& d, const x86::Xmm& s) noexcept {
    if (hasSSSE3()) {
      vswizi8v_(d, s, constAsMem(gConstants.xmm_pshufb_packed_argb32_2x_lo_to_unpacked_a8));
    }
    else {
  ...
  B2D_NOINLINE void xExtractUnpackedAFromPackedARGB32_4(const x86::Vec& d0, const x86::Vec& d1, const x86::Vec& s) noexcept {
    B2D_ASSERT(d0.id() != d1.id());

    if (hasSSSE3()) {
      if (d0.id() == s.id()) {
        vswizi8v_(d1, s, constAsMem(gConstants.xmm_pshufb_packed_argb32_2x_hi_to_unpacked_a8));
        vswizi8v_(d0, s, constAsMem(gConstants.xmm_pshufb_packed_argb32_2x_lo_to_unpacked_a8));
      }
      else {
        vswizi8v_(d0, s, constAsMem(gConstants.xmm_pshufb_packed_argb32_2x_lo_to_unpacked_a8));
        vswizi8v_(d1, s, constAsMem(gConstants.xmm_pshufb_packed_argb32_2x_hi_to_unpacked_a8));
  ...
  B2D_NOINLINE void xPackU32ToU16Lo(const x86::Vec& d0, const x86::Vec& s0) noexcept {
    if (hasSSE4_1()) {
      vpacki32u16_(d0, s0, s0);
    }
    else if (hasSSSE3()) {
      vswizi8v_(d0, s0, constAsMem(gConstants.xmm_pshufb_u32_to_u16_lo));
    }
    else {
  ...
  B2D_NOINLINE void xPackU32ToU16Lo(const VecArray& d0, const VecArray& s0) noexcept {
  ...
  x86::Compiler* cc;                     //!< AsmJit compiler.
  x86::Features _features;               //!< Target CPU features.

  uint32_t _optLevel;                    //!< Optimization level.
  PipeRegUsage _availableRegs;           //!< Number of registers available to the pipeline compiler.
  PipeRegUsage _temporaryRegs;           //!< Estimation of registers used by the pipeline temporarily.
  PipeRegUsage _persistentRegs;          //!< Estimation of registers used by the pipeline permanently.

  asmjit::FuncNode* _funcNode;           //!< Function node.
  asmjit::BaseNode* _funcInit;           //!< Function initialization hook.
  asmjit::BaseNode* _funcEnd;            //!< Function end hook (to add 'unlikely' branches).

  x86::Gp _gpNone;                       //!< Invalid GP register.
  x86::Gp _ctxData;                      //!< Holds `FillFunc::ctxData` argument.
  x86::Gp _fillData;                     //!< Holds `FillFunc::fillData` argument.
  x86::Gp _fetchData;                    //!< Holds `FillFunc::fetchData` argument.
  x86::Mem _tmpStack;                    //!< Temporary stack used to transfer SIMD regs to GP/MM.

  int _ctxDataOffset;                    //!< Offset to get real ctx-data from the passed pointer.
  int _fillDataOffset;                   //!< Offset to get real fill-data from the passed pointer.
  int _fetchDataOffset;                  //!< Offset to get real fetch-data from the passed pointer.

  int32_t _constantsOff;                 //!< Offset to the first constant to the `gConstants` global.
  x86::Gp _constantsPtr;                 //!< Pointer to the `gConstants` constant pool (only used in 64-bit mode).
  x86::Xmm _constantsXmm[4];             //!< XMM constants.
};

// ============================================================================
// [b2d::Pipe2D::PipeInjectAtTheEnd]
// ============================================================================

class PipeInjectAtTheEnd {
public:
  inline PipeInjectAtTheEnd(PipeCompiler* pc) noexcept
    : _injector(pc->cc, &pc->_funcEnd) {}

  PipeScopedInject _injector;
};
...
// [Guard]
#endif // _B2D_PIPE2D_PIPECOMPILER_P_H
