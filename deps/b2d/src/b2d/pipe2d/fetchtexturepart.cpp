...
#define REL_TEXTURE(FIELD) ASMJIT_OFFSET_OF(FetchTextureData, FIELD)

// ============================================================================
// [b2d::Pipe2D::FetchTexturePart - Construction / Destruction]
// ============================================================================

FetchTexturePart::FetchTexturePart(PipeCompiler* pc, uint32_t fetchType, uint32_t fetchExtra, uint32_t pixelFormat) noexcept
  : FetchPart(pc, fetchType, fetchExtra, pixelFormat) {}

// ============================================================================
// [b2d::Pipe2D::FetchTextureSimplePart - Construction / Destruction]
// ============================================================================

FetchTextureSimplePart::FetchTextureSimplePart(PipeCompiler* pc, uint32_t fetchType, uint32_t fetchExtra, uint32_t pixelFormat) noexcept
  : FetchTexturePart(pc, fetchType, fetchExtra, pixelFormat),
    _extendX(0) {
  ...
  static const uint8_t aaExtendTable[] = {
    FetchTextureData::kExtendPad,
    FetchTextureData::kExtendRepeat,
    FetchTextureData::kExtendRoR
  };
  ...
  switch (fetchType) {
    case kFetchTypeTextureAABlit:
      _maxPixels = 8;
      _persistentRegs[x86::Reg::kGroupGp] = 1;
      break;

    case kFetchTypeTextureAAPadX:
      _maxPixels = 8;
    case kFetchTypeTextureAARepeatLargeX:
    case kFetchTypeTextureAARoRX:
      _extendX = aaExtendTable[fetchType - kFetchTypeTextureAAPadX];
      _persistentRegs[x86::Reg::kGroupGp] = 3;
      break;

    case kFetchTypeTextureFxPadX:
    case kFetchTypeTextureFxRoRX:
      _extendX = auExtendTable[fetchType - kFetchTypeTextureFxPadX];
      _persistentRegs[x86::Reg::kGroupGp] = 3;
      _persistentRegs[x86::Reg::kGroupVec] = 1;
      break;

    case kFetchTypeTextureFyPadX:
    case kFetchTypeTextureFyRoRX:
      _extendX = auExtendTable[fetchType - kFetchTypeTextureFyPadX];
      _persistentRegs[x86::Reg::kGroupGp] = 3;
      break;

    case kFetchTypeTextureFxFyPadX:
    case kFetchTypeTextureFxFyRoRX:
      _extendX = auExtendTable[fetchType - kFetchTypeTextureFxFyPadX];
...
// ============================================================================
// [b2d::Pipe2D::FetchTextureSimplePart - Init / Fini]
// ============================================================================

void FetchTextureSimplePart::_initPart(x86::Gp& x, x86::Gp& y) noexcept {
  if (isAABlit()) {
    ...
    cc->mov(f->srcp1, x86::ptr(pc->_fetchData, REL_TEXTURE(src.pixels)));
    cc->mov(f->stride.r32(), x86::ptr(pc->_fetchData, REL_TEXTURE(src.width)));
    pc->uPrefetch(x86::ptr(f->srcp1));

    pc->uMulImm(f->stride, f->stride, -int(bpp()));
    cc->add(f->stride, x86::ptr(pc->_fetchData, REL_TEXTURE(src.stride)));
    cc->spill(f->stride);
  }
  else {
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    cc->mov(f->y, x86::ptr(pc->_fetchData, REL_TEXTURE(simple.ty)));
    cc->add(f->y, y);

    cc->mov(f->srctop, x86::ptr(pc->_fetchData, REL_TEXTURE(src.pixels)));
    cc->spill(f->srctop);

    cc->mov(f->stride, x86::ptr(pc->_fetchData, REL_TEXTURE(src.stride)));
    cc->mov(f->strideOrig, f->stride);

    cc->mov(f->h , x86::ptr(pc->_fetchData, REL_TEXTURE(src.height)));
    cc->mov(f->ry, x86::ptr(pc->_fetchData, REL_TEXTURE(simple.ry)));
    ...
    //      `f->stride` - Real stride copied from the texture data.
    //      `f->y`      - How many scanlines until we reach the bottom of the texture.
    //
    ...
    // reached the width `w`.
    if (extendX() == FetchTextureData::kExtendPad) {
      // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      cc->mov(f->w      , x86::ptr(pc->_fetchData, REL_TEXTURE(src.width)));
      cc->mov(f->xOrigin, x86::ptr(pc->_fetchData, REL_TEXTURE(simple.tx)));
    ...
    // ----------------------------------------
    //
    // This extend mode is only used to blit textures that are tiled and that
    // exceed some predefined width-limit (like 16|32|etc). It's specialized
    // for larger textures because it contains a condition in fetchN() that
    // jumps if `f->x` is at the end (or near it) of the texture. That's why
    // the texture width should be large enough that this branch is not
    // mispredicted often. For smaller textures RoR more is more suitable as
    ...
    //
    //   2. It multiplies X coordinates (all of them) by texture's BPP (bytes
    ...
    if (extendX() == FetchTextureData::kExtendRepeat) {
      ...
      cc->mov(f->w            , x86::ptr(pc->_fetchData, REL_TEXTURE(src.width)));
      cc->mov(f->xOrigin.r32(), x86::ptr(pc->_fetchData, REL_TEXTURE(simple.tx)));
    ...
    //      texture width it's simply corrected as `f->x -= f->rx`, where `f->rx`
    //      is equal to `texture.width`.
    //
    //   2. REFLECT - X is always bound to interval [-Width...Width) so it
    //      can reflect. When `f->x` reaches the texture width it's simply
    //      corrected as `f->x -= f->rx`, where `f->rx` is equal to `texture.width * 2`
    //      so it goes negative.
    if (extendX() == FetchTextureData::kExtendRoR) {
      // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      f->x          = cc->newInt32("f.x");            // Reg.
      f->xOrigin    = cc->newInt32("f.xOrigin");      // Mem.
      ...
      cc->mov(f->w , x86::ptr(pc->_fetchData, REL_TEXTURE(src.width)));
      cc->mov(f->rx, x86::ptr(pc->_fetchData, REL_TEXTURE(simple.rx)));
      ...
      if (maxPixels() >= 4) {
        ...
        pc->vmovu8u32(f->xSet4, x86::ptr(pc->_fetchData, REL_TEXTURE(simple.ix)));
    ...
    if (isAUTexture()) {
      ...
      pc->vloadi128u(weights, x86::ptr(pc->_fetchData, REL_TEXTURE(simple.wa))); // [00 Wd 00 Wc 00 Wb 00 Wa]
...
void FetchTextureSimplePart::_finiPart() noexcept {}

// ============================================================================
// [b2d::Pipe2D::FetchTextureSimplePart - Advance]
// ============================================================================

void FetchTextureSimplePart::advanceY() noexcept {
  if (isAABlit()) {
    // Blit AA
    // -------
    ...
    // If this texture fetch uses two source pointers (one for current scanline
...
void FetchTextureSimplePart::startAtX(x86::Gp& x) noexcept {
  if (isAABlit()) {
    // Blit AA
    // -------
    ...
    if (extendX() == FetchTextureData::kExtendRepeat) {
      if (!isRectFill()) {                                 // if (!RectFill) {
        pc->uAddMulImm(f->x, x, int(bpp()));               //   f->x += x * texture.bpp;
...
void FetchTextureSimplePart::advanceX(x86::Gp& x, x86::Gp& diff) noexcept {
  B2D_UNUSED(x);
  ...
  else if (extendX() == FetchTextureData::kExtendPad) {
    // Horizontal Pad
    // --------------
  ...
  else if (extendX() == FetchTextureData::kExtendRepeat) {
    // Horizontal Repeat - AA-Only, Large Fills
    // ----------------------------------------

    pc->uAddMulImm(f->x, diff, int(bpp()));                // f->x += diff * texture.bpp;
    repeatOrReflectX();                                    // f->x = repeatLarge(f->x);
  }
  else if (extendX() == FetchTextureData::kExtendRoR) {
    // Horizontal RoR [Repeat or Reflect]
    // ----------------------------------
...
void FetchTextureSimplePart::advanceXByOne() noexcept {
  if (isAABlit()) {
    // Blit AA
    // -------
...
void FetchTextureSimplePart::repeatOrReflectX() noexcept {
  if (isAABlit()) {
    // Blit AA
    // -------
...
void FetchTextureSimplePart::prefetchAccX() noexcept {
  B2D_ASSERT(hasFracX());
...
// ============================================================================
// [b2d::Pipe2D::FetchTextureSimplePart - Fetch]
// ============================================================================

void FetchTextureSimplePart::fetch1(PixelARGB& p, uint32_t flags) noexcept {
  if (isAABlit()) {
    // Blit AA
    // -------
...
void FetchTextureSimplePart::enterN() noexcept {
  if (isAABlit()) {
    // Blit AA
    // -------
...
void FetchTextureSimplePart::leaveN() noexcept {
  if (isAABlit()) {
    // Blit AA
    // -------
...
void FetchTextureSimplePart::prefetchN() noexcept {}
void FetchTextureSimplePart::postfetchN() noexcept {}

void FetchTextureSimplePart::fetch4(PixelARGB& p, uint32_t flags) noexcept {
  if (isAABlit()) {
    // Blit AA
    // -------
    ...
    if (extendX() == FetchTextureData::kExtendPad) {
      ...
      if (isFyTexture()) {
        ...
        Fetch4Context fCtx0(pc, &pix0, PixelARGB::kUC);
        Fetch4Context fCtx1(pc, &pix1, PixelARGB::kUC);
    ...
    if (extendX() == FetchTextureData::kExtendRepeat) {
      // Only generated for AA textures.
      B2D_ASSERT(isAATexture());
...
void FetchTextureSimplePart::fetch8(PixelARGB& p, uint32_t flags) noexcept {
  FetchPart::fetch8(p, flags);
}

// ============================================================================
// [b2d::Pipe2D::FetchTextureAffinePart - Construction / Destruction]
// ============================================================================

FetchTextureAffinePart::FetchTextureAffinePart(PipeCompiler* pc, uint32_t fetchType, uint32_t fetchExtra, uint32_t pixelFormat) noexcept
  : FetchTexturePart(pc, fetchType, fetchExtra, pixelFormat) {
  ...
  switch (fetchType) {
    case kFetchTypeTextureAffineNnAny:
    case kFetchTypeTextureAffineNnOpt:
      _isComplexFetch = true;
      _persistentRegs[x86::Reg::kGroupVec] = 3;
      break;

    case kFetchTypeTextureAffineBiAny:
    case kFetchTypeTextureAffineBiOpt:
...
// ============================================================================
// [b2d::Pipe2D::FetchTextureAffinePart - Init / Fini]
// ============================================================================

void FetchTextureAffinePart::_initPart(x86::Gp& x, x86::Gp& y) noexcept {
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  f->srctop         = cc->newIntPtr("f.srctop");      // Mem.
  f->stride         = cc->newIntPtr("f.stride");      // Mem.
  ...
  pc->vloadi128u(f->xx_xy, x86::ptr(pc->_fetchData, REL_TEXTURE(affine.xx)));
  pc->vloadi128u(f->yx_yy, x86::ptr(pc->_fetchData, REL_TEXTURE(affine.yx)));

  pc->vmovsi32(f->tx_ty, y);
  pc->vswizi32(f->tx_ty, f->tx_ty, x86::Predicate::shuf(1, 0, 1, 0));
  pc->vMulU64xU32Lo(f->tx_ty, f->yx_yy, f->tx_ty);
  pc->vaddi64(f->tx_ty, f->tx_ty, x86::ptr(pc->_fetchData, REL_TEXTURE(affine.tx)));

  // RoR: `tw_th` and `rx_ry` are only used by repeated or reflected patterns.
  pc->vloadi128u(f->rx_ry, x86::ptr(pc->_fetchData, REL_TEXTURE(affine.rx)));
  pc->vloadi128u(f->tw_th, x86::ptr(pc->_fetchData, REL_TEXTURE(affine.tw)));

  pc->vloadi128u(f->ox_oy, x86::ptr(pc->_fetchData, REL_TEXTURE(affine.ox)));
  pc->vloadi128u(f->rx_ry, x86::ptr(pc->_fetchData, REL_TEXTURE(affine.rx)));
  pc->vloadi128u(f->xx2_xy2, x86::ptr(pc->_fetchData, REL_TEXTURE(affine.xx2)));

  // Pad: [MaxY | MaxX | MinY | MinX]
  pc->vloadi128u(f->minx_miny, x86::ptr(pc->_fetchData, REL_TEXTURE(affine.minX)));
  ...
  // vAddrMul.
  if (isOptimized()) {
    pc->vloadi32(f->vAddrMul, x86::ptr(pc->_fetchData, REL_TEXTURE(affine.addrMul)));
...
void FetchTextureAffinePart::_finiPart() noexcept {}

// ============================================================================
// [b2d::Pipe2D::FetchTextureAffinePart - Advance]
// ============================================================================

void FetchTextureAffinePart::advanceY() noexcept {
...
void FetchTextureAffinePart::startAtX(x86::Gp& x) noexcept {
  if (isRectFill()) {
...
void FetchTextureAffinePart::advanceX(x86::Gp& x, x86::Gp& diff) noexcept {
  B2D_UNUSED(x);
  B2D_ASSERT(!isRectFill());
...
void FetchTextureAffinePart::advancePxPy(x86::Xmm& px_py, const x86::Gp& i) noexcept {
...
void FetchTextureAffinePart::normalizePxPy(x86::Xmm& px_py) noexcept {
...
// ============================================================================
// [b2d::Pipe2D::FetchTextureAffinePart - Fetch]
// ============================================================================

void FetchTextureAffinePart::prefetch1() noexcept {
  x86::Xmm vIdx = f->vIdx;

  switch (fetchType()) {
    case kFetchTypeTextureAffineNnAny: {
      clampVIdxMin32(vIdx, f->px_py, f->minx_miny);
      clampVIdxMax32(vIdx, vIdx, f->maxx_maxy);
      break;
    }
...
void FetchTextureAffinePart::fetch1(PixelARGB& p, uint32_t flags) noexcept {
  int shift = 2;
  ...
  switch (fetchType()) {
    ...
    case kFetchTypeTextureAffineBiAny: {
      ...
      pc->vsubi32(vIdx, vIdx, pc->constAsMem(gConstants.xmm_u32_FFFFFFFF_0));
      pc->vaddi64(f->px_py, f->px_py, f->xx_xy);

      clampVIdxMin32(vIdx, vIdx, f->minx_miny);
      pc->vcmpgti32(vMsk1, f->px_py, f->ox_oy);

      clampVIdxMax32(vIdx, vIdx, f->maxx_maxy);
      pc->vswizhi16(vWeights, vWeights, x86::Predicate::shuf(1, 1, 1, 1));

      pc->vcmpgti32(vMsk0, vIdx, f->ox_oy);
      pc->vand(vMsk1, vMsk1, f->rx_ry);

      pc->vand(vMsk0, vMsk0, f->rx_ry);
      pc->vsrli16(vWeights, vWeights, 8);

      pc->vsubi32(vIdx, vIdx, vMsk0);
      pc->vsubi32(f->px_py, f->px_py, vMsk1);

      pc->vsrai32(vMsk0, vIdx, 31);
      pc->vxor(vWeights, vWeights, pc->constAsMem(gConstants.xmm_u32_FFFFFFFF_0));

      pc->vxor(vIdx, vIdx, vMsk0);
      pc->vaddi16(vWeights, vWeights, pc->constAsMem(gConstants.xmm_u64_0101010100000000));
    ...
    case kFetchTypeTextureAffineBiOpt: {
      // TODO: Not implemented, not used for now...
      break;
    }
  }
}

void FetchTextureAffinePart::enterN() noexcept {
  x86::Xmm vMsk0 = cc->newXmm("vMsk0");
...
void FetchTextureAffinePart::leaveN() noexcept {}

void FetchTextureAffinePart::prefetchN() noexcept {
  switch (fetchType()) {
    case kFetchTypeTextureAffineNnOpt: {
      x86::Xmm vIdx = f->vIdx;
      x86::Xmm vMsk0 = cc->newXmm("vMsk0");
...
void FetchTextureAffinePart::postfetchN() noexcept {
  switch (fetchType()) {
    case kFetchTypeTextureAffineNnOpt: {
      break;
    }
...
void FetchTextureAffinePart::fetch4(PixelARGB& p, uint32_t flags) noexcept {
  int shift = 2;
  ...
  switch (fetchType()) {
    case kFetchTypeTextureAffineNnAny: {
      ...
      pc->vaddi64(f->px_py, f->px_py, f->xx2_xy2);
      pc->vaddi64(f->qx_qy, f->qx_qy, f->xx2_xy2);
      clampVIdxMin32(vIdx0, vIdx0, f->minx_miny);

      pc->vcmpgti32(vMsk0, f->px_py, f->ox_oy);
      pc->vcmpgti32(vMsk1, f->qx_qy, f->ox_oy);
      clampVIdxMax32(vIdx0, vIdx0, f->maxx_maxy);

      pc->vand(vMsk0, vMsk0, f->rx_ry);
      pc->vand(vMsk1, vMsk1, f->rx_ry);

      pc->vsubi32(f->px_py, f->px_py, vMsk0);
      pc->vsrai32(vMsk0, vIdx0, 31);

      pc->vsubi32(f->qx_qy, f->qx_qy, vMsk1);
      pc->vxor(vIdx0, vIdx0, vMsk0);
      iExt.begin(vIdx0);
...
void FetchTextureAffinePart::clampVIdxMin32(x86::Xmm& dst, const x86::Xmm& a, const x86::Xmm& b) noexcept {
  B2D_ASSERT(a.id() != b.id());
  ...
  if (pc->hasSSE4_1()) {
    ...
    // We know that `b` (minx_miny) can contain these values (per vector element):
    //
    //   a) `b == 0` to handle PAD case.
    //   b) `b == INT32_MIN` to handle REPEAT/REFLECT cases.
    //
    // This means that we don't have to provide a solution that would be fully
    // compliant with `PMAXSD` instruction as we only need a solution that would
    // work with the values `b` can contain.
    if (dst.id() == a.id()) {
      x86::Xmm tmp = cc->newXmm("tmp");
      pc->vmov(tmp, dst);
      pc->vcmpgti32(dst, dst, b); // `0xFFFFFFFF` if `a` is greater than `b`.
      pc->vand(dst, dst, tmp);    // Changes `dst` to `0` if clamped or keeps it as is if `a` > `b`.
    }
    else {
      pc->vmov(dst, a);
      pc->vcmpgti32(dst, dst, b); // `0xFFFFFFFF` if `a` is greater than `b`.
      pc->vand(dst, dst, a);      // Changes `dst` to `0` if clamped or keeps it as is if `a` > `b`.
    }
  }
}

void FetchTextureAffinePart::clampVIdxMax32(x86::Xmm& dst, const x86::Xmm& a, const x86::Xmm& b) noexcept {
  B2D_ASSERT(a.id() != b.id());
