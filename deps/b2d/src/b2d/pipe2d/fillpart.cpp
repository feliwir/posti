...
// ============================================================================
// [b2d::Pipe2D::FillPart - Construction / Destruction]
// ============================================================================
...
// ============================================================================
// [b2d::Pipe2D::FillBoxAAPart - Construction / Destruction]
// ============================================================================
...
// ============================================================================
// [b2d::Pipe2D::FillBoxAAPart - Compile]
// ============================================================================
...
void FillBoxAAPart::compile() noexcept {
  ...
  cc->mov(dstPtr, cc->intptr_ptr(ctxData, REL(ContextData, dst.stride)));
  cc->mov(y, x86::dword_ptr(fillData, REL(FillData::BoxAA, box._y0)));

  cc->mov(dstStride, dstPtr);
  cc->mov(w, x86::dword_ptr(fillData, REL(FillData::BoxAA, box._x0)));
  ...
  cc->add(dstPtr, cc->intptr_ptr(ctxData, REL(ContextData, dst.pixels)));
  cc->add(w, x86::dword_ptr(fillData, REL(FillData::BoxAA, box._x1)));

  pc->uMulImm(x, w, dstBpp);
  cc->add(y, x86::dword_ptr(fillData, REL(FillData::BoxAA, box._y1)));
  ...
  if (compOpPart()->shouldOptimizeOpaqueFill()) {
    ...
    cc->mov(m, x86::dword_ptr(fillData, REL(FillData::BoxAA, alpha)));
    cc->cmp(m, 255);
    ...
    compOpPart()->cMaskInit(x86::dword_ptr(fillData, REL(FillData::BoxAA, alpha)));
...
// ============================================================================
// [b2d::Pipe2D::FillBoxAUPart - Construction / Destruction]
// ============================================================================
...
// ============================================================================
// [b2d::Pipe2D::FillBoxAUPart - Compile]
// ============================================================================
...
void FillBoxAUPart::compile() noexcept {
  ...
  cc->mov(dstPtr, cc->intptr_ptr(ctxData, REL(ContextData, dst.stride)));
  cc->mov(h, x86::dword_ptr(fillData, REL(FillData::BoxAU, box._y0)));

  cc->mov(dstStride, dstPtr);
  cc->mov(innerWidth, x86::dword_ptr(fillData, REL(FillData::BoxAU, box._x0)));
  ...
  cc->add(dstPtr, cc->intptr_ptr(ctxData, REL(ContextData, dst.pixels)));
  cc->add(innerWidth, x86::dword_ptr(fillData, REL(FillData::BoxAU, box._x1)));

  pc->uMulImm(x, innerWidth, dstBpp);
  cc->add(h, x86::dword_ptr(fillData, REL(FillData::BoxAU, box._y1)));
  cc->sub(dstStride, x.cloneAs(dstStride));

  cc->mov(startWidth, x86::dword_ptr(fillData, REL(FillData::BoxAU, startWidth)));
  cc->mov(innerWidth, x86::dword_ptr(fillData, REL(FillData::BoxAU, innerWidth)));
  ...
  cc->lea(pMasks, x86::ptr(fillData, REL(FillData::BoxAU, masks)));
...
// ============================================================================
// [b2d::Pipe2D::FillAnalyticPart - Construction / Destruction]
// ============================================================================
...
// ============================================================================
// [b2d::Pipe2D::FillAnalyticPart - Compile]
// ============================================================================
...
void FillAnalyticPart::compile() noexcept {
  ...
  int dstBpp = int(dstPart()->bpp());
  int bwSize = int(sizeof(Support::BitWord));
  int bwSizeInBits = bwSize * 8;

  int pixelsPerOneBit = 4;
  int pixelsPerOneBitShift = Support::ctz(pixelsPerOneBit);
  ...
  cc->mov(y, x86::dword_ptr(fillData, REL(FillData::Analytic, box._y0)));
  cc->mov(dstStride, cc->intptr_ptr(ctxData, REL(ContextData, dst.stride)));
  ...
  cc->mov(bitStride , cc->intptr_ptr(fillData, REL(FillData::Analytic, bitStride)));
  cc->mov(cellStride, cc->intptr_ptr(fillData, REL(FillData::Analytic, cellStride)));

  cc->spill(bitStride);
  cc->spill(cellStride);

  cc->mov(bitPtr, cc->intptr_ptr(fillData, REL(FillData::Analytic, bitTopPtr)));
  cc->mov(cellPtr, cc->intptr_ptr(fillData, REL(FillData::Analytic, cellTopPtr)));
  ...
  cc->neg(y);
  cc->add(y, x86::dword_ptr(fillData, REL(FillData::Analytic, box._y1)));

  // xEnd = fillData->box.x1;
  cc->mov(xEnd, x86::dword_ptr(fillData, REL(FillData::Analytic, box._x1)));
  cc->spill(xEnd);

  pc->vloadi32(globalAlpha, x86::dword_ptr(fillData, REL(FillData::Analytic, alpha)));
  ...
  pc->vloadi32(fillRuleMask, x86::dword_ptr(fillData, REL(FillData::Analytic, fillRuleMask)));
  ...
  pc->vloadi128a(cov, pc->constAsMem(gConstants.xmm_u32_00020000)); // cov[3:0] = 256 << 9;
  ...
  if (compOpPart()->maxPixels() < 4) {
    ...
    // We must use unaligned loads here as we don't whether we are at the end
    // of the scanline. In that case `cellPtr` might already be misaligned if
    // the image width is not divisible by 4.
    ...
    accumulateCovers(cov, m[0]);
    calcMasksFromCovers(m[0], m[0], fillRuleMask, globalAlpha, false);
  ...
  if (compOpPart()->shouldOptimizeOpaqueFill()) {
    ...
    cc->cmp(cMaskAlpha, 255);                              //   if (cMaskAlpha != 255)
  ...
  // TODO: Verify whether it's necessary.
...
void FillAnalyticPart::accumulateCovers(const x86::Vec& acc, const x86::Vec& val) noexcept {
...
void FillAnalyticPart::calcMasksFromCovers(const x86::Vec& dst, const x86::Vec& src, const x86::Vec& fillRuleMask, const x86::Vec& globalAlpha, bool unpack) noexcept {
  pc->vsrai32(dst, src, A8Consts::kA8Shift + 1);
  pc->vand(dst, dst, fillRuleMask);
  pc->vsubi32(dst, dst, pc->constAsMem(gConstants.xmm_u32_00000100));
  pc->vabsi32(dst, dst);

  pc->vpacki32i16(dst, dst, dst);
  pc->vmini16(dst, dst, pc->constAsMem(gConstants.xmm_u16_00FF));

  // Now we have a vector of 16-bit masks:
  //
  //   [__, __, __, __, M3, M2, M1, M0]
  //
  // After unpacking (if enabled) we would get a vector of 16-bit masks:
  //
  ...
  // Multiply masks by global alpha, if given...
  if (globalAlpha.isValid()) {
    pc->vmulu16(dst, dst, globalAlpha);
    pc->vsrli16(dst, dst, 8);
  }
}
