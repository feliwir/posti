...
// ============================================================================
// [b2d::Pipe2D::PipeCompiler - Construction / Destruction]
// ============================================================================

PipeCompiler::PipeCompiler(x86::Compiler* cc) noexcept : cc(cc) { reset(); }
PipeCompiler::~PipeCompiler() noexcept {}

// ============================================================================
// [b2d::Pipe2D::PipeCompiler - Reset]
// ============================================================================
...
// ============================================================================
// [b2d::Pipe2D::PipeCompiler - Optimization Level]
// ============================================================================

void PipeCompiler::_detectFeatures() noexcept {
  using namespace asmjit;
  const CpuInfo& cpuInfo = CpuInfo::host();

  _features = cpuInfo.features<x86::Features>();
  _optLevel = kPipeOpt_X86_SSE2;

  // TODO: No AVX2 use at the moment.
  if (cpuInfo.hasFeature(x86::Features::kSSE3  )) _optLevel = kPipeOpt_X86_SSE3;
  if (cpuInfo.hasFeature(x86::Features::kSSSE3 )) _optLevel = kPipeOpt_X86_SSSE3;
  if (cpuInfo.hasFeature(x86::Features::kSSE4_1)) _optLevel = kPipeOpt_X86_SSE4_1;
  if (cpuInfo.hasFeature(x86::Features::kSSE4_2)) _optLevel = kPipeOpt_X86_SSE4_2;
  if (cpuInfo.hasFeature(x86::Features::kAVX   )) _optLevel = kPipeOpt_X86_AVX;
}

// ============================================================================
// [b2d::Pipe2D::PipeCompiler - BeginFunction / EndFunction]
// ============================================================================
...
void PipeCompiler::endFunction() noexcept {
  // All pipelines return zero, which means `kErrorOk`.
...
// ============================================================================
// [b2d::Pipe2D::PipeCompiler - Parts]
// ============================================================================

FillPart* PipeCompiler::newFillPart(uint32_t fillType, FetchPart* dstPart, CompOpPart* compOpPart) noexcept {
  if (fillType == kFillTypeBoxAA)
    return newPartT<FillBoxAAPart>(fillType, dstPart->as<FetchPixelPtrPart>(), compOpPart);

  if (fillType == kFillTypeBoxAU)
    return newPartT<FillBoxAUPart>(fillType, dstPart->as<FetchPixelPtrPart>(), compOpPart);

  if (fillType == kFillTypeAnalytic)
    return newPartT<FillAnalyticPart>(fillType, dstPart->as<FetchPixelPtrPart>(), compOpPart);

  return nullptr;
}

FetchPart* PipeCompiler::newFetchPart(uint32_t fetchType, uint32_t fetchExtra, uint32_t pixelFormat) noexcept {
  if (fetchType == kFetchTypeSolid)
    return newPartT<FetchSolidPart>(fetchType, fetchExtra, pixelFormat);

  if (fetchType >= _kFetchTypeGradientLinearFirst && fetchType <= _kFetchTypeGradientLinearLast)
    return newPartT<FetchGradientLinearPart>(fetchType, fetchExtra, pixelFormat);

  if (fetchType >= _kFetchTypeGradientRadialFirst && fetchType <= _kFetchTypeGradientRadialLast)
    return newPartT<FetchGradientRadialPart>(fetchType, fetchExtra, pixelFormat);

  if (fetchType >= _kFetchTypeGradientConicalFirst && fetchType <= _kFetchTypeGradientConicalLast)
    return newPartT<FetchGradientConicalPart>(fetchType, fetchExtra, pixelFormat);

  if (fetchType >= _kFetchTypeTextureSimpleFirst && fetchType <= _kFetchTypeTextureSimpleLast)
    return newPartT<FetchTextureSimplePart>(fetchType, fetchExtra, pixelFormat);

  if (fetchType >= _kFetchTypeTextureAffineFirst && fetchType <= _kFetchTypeTextureAffineLast)
    return newPartT<FetchTextureAffinePart>(fetchType, fetchExtra, pixelFormat);

  if (fetchType == kFetchTypePixelPtr)
    return newPartT<FetchPixelPtrPart>(fetchType, fetchExtra, pixelFormat);
...
// ============================================================================
// [b2d::Pipe2D::PipeCompiler - Constants]
// ============================================================================

void PipeCompiler::_initConstantsPtr() noexcept {
  const void* global = &gConstants;

  if (!_constantsPtr.isValid()) {
    asmjit::BaseNode* prevNode = cc->setCursor(_funcInit);
    _constantsPtr = cc->newIntPtr("constants");

    cc->alloc(_constantsPtr);
    cc->mov(_constantsPtr, (int64_t)global + _constantsOff);
...
x86::Mem PipeCompiler::constAsMem(const void* p) noexcept {
  // Make sure we are addressing a constant from the `gConstants` constant pool.
  const void* global = &gConstants;
  B2D_ASSERT((uintptr_t)p >= (uintptr_t)global &&
                 (uintptr_t)p <  (uintptr_t)global + sizeof(Constants));
...
x86::Xmm PipeCompiler::constAsXmm(const void* p) noexcept {
  ...
  if      (p == gConstants.xmm_zeros   ) constIndex = 0; // Required if the CPU doesn't have SSE4.1.
  else if (p == gConstants.xmm_u16_0080) constIndex = 1; // Required by `xDiv255()` and friends.
  else if (p == gConstants.xmm_u16_0101) constIndex = 2; // Required by `xDiv255()` and friends.

  if (constIndex == -1 || _persistentRegs[x86::Reg::kGroupVec] + _temporaryRegs[x86::Reg::kGroupVec] > _availableRegs[x86::Reg::kGroupVec]) {
    // TODO: This works, but it's really nasty!
  ...
  if (!xmm.isValid()) {
    ...
    if (constIndex == 0) {
      ...
      // `constAsMem()` may call `_initConstantsPtr()` for the very first time.
...
// ============================================================================
// [b2d::Pipe2D::PipeCompiler - Stack]
// ============================================================================

x86::Mem PipeCompiler::tmpStack(uint32_t size) noexcept {
  B2D_ASSERT(Support::isPowerOf2(size));

  // TODO: We don't use greater right now.
  B2D_ASSERT(size <= 16);
  ...
  if (PackedInst::isIntrin(packedId)) {
    switch (PackedInst::intrinId(packedId)) {
      ...
      case kIntrin2Vmovu8u16: {
        ...
        vunpackli8(dst, dst, constAsXmm(gConstants.xmm_zeros));
        return;
      }
      ...
      case kIntrin2Vmovu8u32: {
        ...
        vunpackli8(dst, dst, constAsXmm(gConstants.xmm_zeros));
        vunpackli16(dst, dst, constAsXmm(gConstants.xmm_zeros));
        return;
      }
      ...
      case kIntrin2Vinv255u16: {
        Operand u16_255 = constAsXmm(gConstants.xmm_u16_00FF);
      ...
      case kIntrin2Vinv256u16: {
        x86::Vec u16_0100 = constAsXmm(gConstants.xmm_u16_0100);
        ...
        if (!isSameReg(dst, src)) {
          ...
          vxor(dst, dst, constAsXmm(gConstants.xmm_ones));
      ...
      case kIntrin2Vinv255u32: {
        Operand u32_255 = constAsXmm(gConstants.xmm_u32_000000FF);
      ...
      default:
        B2D_ASSERT(!"Invalid intrinsic at vemit_vv_vv()");
    }
  }
...
void PipeCompiler::vemit_vvi_vi(uint32_t packedId, const Operand_& dst_, const Operand_& src_, int imm) noexcept {
  ...
  if (PackedInst::isIntrin(packedId)) {
    switch (PackedInst::intrinId(packedId)) {
      ...
      default:
        B2D_ASSERT(!"Invalid intrinsic at vemit_vvi_vi()");
    }
  }
...
void PipeCompiler::vemit_vvv_vv(uint32_t packedId, const Operand_& dst_, const Operand_& src1_, const Operand_& src2_) noexcept {
  ...
  if (PackedInst::isIntrin(packedId)) {
    switch (PackedInst::intrinId(packedId)) {
      ...
      default:
        B2D_ASSERT(!"Invalid intrinsic at vemit_vvv_vv()");
    }
  }
...
// ============================================================================
// [b2d::Pipe2D::PipeCompiler - Emit - 'Q' Vector Instructions (64-bit MMX)]
// ============================================================================
...
// ============================================================================
// [b2d::Pipe2D::PipeCompiler - Fetch Helpers]
// ============================================================================
...
// ============================================================================
// [b2d::Pipe2D::PipeCompiler - Commons]
// ============================================================================
...
void PipeCompiler::xInlineMemcpyXmm(
  ...
  // TODO: Don't create registers we don't need, there should be loop that only creates
  // enough registers for the memcpy.
  t[0] = cc->newXmm("t0");
  t[1] = cc->newXmm("t1");
  t[2] = cc->newXmm("t2");
  t[3] = cc->newXmm("t3");
  ...
  int n = numBytes / 16;
  do {
    int a, b = Math::min<int>(numBytes, int(ASMJIT_ARRAY_SIZE(t)));
