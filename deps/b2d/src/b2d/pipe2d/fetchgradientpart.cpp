...
#define REL ASMJIT_OFFSET_OF
#define REL_GRADIENT(field) ASMJIT_OFFSET_OF(FetchGradientData, field)

// ============================================================================
// [b2d::Pipe2D::FetchGradientPart - Construction / Destruction]
// ============================================================================

FetchGradientPart::FetchGradientPart(PipeCompiler* pc, uint32_t fetchType, uint32_t fetchExtra, uint32_t pixelFormat) noexcept
  : FetchPart(pc, fetchType, fetchExtra, pixelFormat),
    _extend(0) {}

// ============================================================================
// [b2d::Pipe2D::FetchGradientLinearPart - Construction / Destruction]
// ============================================================================

FetchGradientLinearPart::FetchGradientLinearPart(PipeCompiler* pc, uint32_t fetchType, uint32_t fetchExtra, uint32_t pixelFormat) noexcept
  : FetchGradientPart(pc, fetchType, fetchExtra, pixelFormat),
    _isRoR(fetchType == kFetchTypeGradientLinearRoR) {
  ...
  _extend = uint8_t(fetchType - kFetchTypeGradientLinearPad);

  JitUtils::resetVarStruct(&f, sizeof(f));
}

// ============================================================================
// [b2d::Pipe2D::FetchGradientLinearPart - Init / Fini]
// ============================================================================

void FetchGradientLinearPart::_initPart(x86::Gp& x, x86::Gp& y) noexcept {
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  cc->mov(f->table, x86::ptr(pc->_fetchData, REL_GRADIENT(table.data)));
  ...
  if (isPad() && !pc->hasSSE4_1()) {
    pc->vsubi32(f->py, f->py, pc->constAsMem(gConstants.xmm_u64_0000080000000800));
    pc->vsubi32(f->msk, f->msk, pc->constAsMem(gConstants.xmm_u16_8000));
  }
...
void FetchGradientLinearPart::_finiPart() noexcept {}

// ============================================================================
// [b2d::Pipe2D::FetchGradientLinearPart - Advance]
// ============================================================================

void FetchGradientLinearPart::advanceY() noexcept {
  pc->vaddi64(f->py, f->py, f->dy);
}

void FetchGradientLinearPart::startAtX(x86::Gp& x) noexcept {
  pc->vmov(f->pt, f->py);
...
void FetchGradientLinearPart::advanceX(x86::Gp& x, x86::Gp& diff) noexcept {
  B2D_UNUSED(x);
...
// ============================================================================
// [b2d::Pipe2D::FetchGradientLinearPart - Fetch]
// ============================================================================

void FetchGradientLinearPart::prefetch1() noexcept {
  if (isPad()) {
    // Nothing...
  }
  else {
...
void FetchGradientLinearPart::fetch1(PixelARGB& p, uint32_t flags) noexcept {
  ...
  if (isPad()) {
    if (pc->hasSSE4_1()) {
      ...
      pc->vmini16(vTmp, vTmp, f->msk);
      pc->vaddi16(vTmp, vTmp, pc->constAsMem(gConstants.xmm_u16_8000));
      pc->vaddi64(f->pt, f->pt, f->dt);
...
void FetchGradientLinearPart::enterN() noexcept {}
void FetchGradientLinearPart::leaveN() noexcept {}

void FetchGradientLinearPart::prefetchN() noexcept {
  x86::Xmm vIdx = f->vIdx;
...
void FetchGradientLinearPart::postfetchN() noexcept {
  pc->vsubi64(f->pt, f->pt, f->dt2);
  pc->vsubi64(f->pt, f->pt, f->dt2);
}

void FetchGradientLinearPart::fetch4(PixelARGB& p, uint32_t flags) noexcept {
  IndexExtractorU16 iExt(pc, IndexExtractorU16::kStrategyStack);
  Fetch4Context fCtx(pc, &p, flags);

  x86::Gp tIdx0 = cc->newInt32("tIdx0");
  x86::Gp tIdx1 = cc->newInt32("tIdx1");
  ...
  if (isPad()) {
    if (pc->hasSSE4_1()) {
      ...
      pc->vmini16(vIdx, vIdx, f->msk);
      pc->vaddi16(vIdx, vIdx, pc->constAsMem(gConstants.xmm_u16_8000));
      iExt.begin(vIdx);
...
void FetchGradientLinearPart::fetch8(PixelARGB& p, uint32_t flags) noexcept {
  IndexExtractorU16 iExt(pc, IndexExtractorU16::kStrategyStack);
  Fetch8Context fCtx(pc, &p, flags);
...
// ============================================================================
// [b2d::Pipe2D::FetchGradientRadialPart - Construction / Destruction]
// ============================================================================

FetchGradientRadialPart::FetchGradientRadialPart(PipeCompiler* pc, uint32_t fetchType, uint32_t fetchExtra, uint32_t pixelFormat) noexcept
  : FetchGradientPart(pc, fetchType, fetchExtra, pixelFormat) {
  ...
  _extend = uint8_t(fetchType - kFetchTypeGradientRadialPad);

  JitUtils::resetVarStruct(&f, sizeof(f));
}

// ============================================================================
// [b2d::Pipe2D::FetchGradientRadialPart - Init / Fini]
// ============================================================================

void FetchGradientRadialPart::_initPart(x86::Gp& x, x86::Gp& y) noexcept {
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  f->table          = cc->newIntPtr("f.table");       // Reg.
...
void FetchGradientRadialPart::_finiPart() noexcept {}

// ============================================================================
// [b2d::Pipe2D::FetchGradientRadialPart - Advance]
// ============================================================================

void FetchGradientRadialPart::advanceY() noexcept {
  pc->vaddpd(f->px_py, f->px_py, f->yx_yy);
}

void FetchGradientRadialPart::startAtX(x86::Gp& x) noexcept {
  if (isRectFill()) {
    precalc(f->px_py);
  }
  else {
...
void FetchGradientRadialPart::advanceX(x86::Gp& x, x86::Gp& diff) noexcept {
  B2D_UNUSED(diff);
...
// ============================================================================
// [b2d::Pipe2D::FetchGradientRadialPart - Fetch]
// ============================================================================

void FetchGradientRadialPart::prefetch1() noexcept {
  pc->vcvtpdps(f->value, f->d_b);
  pc->vandps(f->value, f->value, pc->constAsMem(gConstants.xmm_f_abs_lo));
  pc->vsqrtss(f->value, f->value, f->value);
}

void FetchGradientRadialPart::fetch1(PixelARGB& p, uint32_t flags) noexcept {
  ...
  if (extend() == Gradient::kExtendPad) {
    pc->vmaxss(x0, x0, pc->constAsXmm(gConstants.xmm_zeros));
  ...
  if (extend() == Gradient::kExtendRepeat) {
    cc->and_(idx, f->maxi);
  }

  if (extend() == Gradient::kExtendReflect) {
...
void FetchGradientRadialPart::prefetchN() noexcept {
  x86::Xmm& d_b   = f->d_b;
  ...
  pc->vandps(value, value, pc->constAsMem(gConstants.xmm_f_abs));
...
void FetchGradientRadialPart::postfetchN() noexcept {
  pc->vmovaps(f->d_b, f->d_b_prev);     // Restore `d_b`.
  pc->vmovaps(f->dd_bd, f->dd_bd_prev); // Restore `dd_bd`.
}

void FetchGradientRadialPart::fetch4(PixelARGB& p, uint32_t flags) noexcept {
  x86::Xmm& d_b   = f->d_b;
  x86::Xmm& dd_bd = f->dd_bd;
  x86::Xmm& ddd   = f->ddd;
  x86::Xmm& value = f->value;
  ...
  if (extend() == Gradient::kExtendPad)
    pc->vmaxps(value, value, pc->constAsXmm(gConstants.xmm_zeros));
  ...
  pc->vandps(x0, x0, pc->constAsMem(gConstants.xmm_f_abs));
...
void FetchGradientRadialPart::precalc(x86::Xmm& px_py) noexcept {
  x86::Xmm& d_b   = f->d_b;
  x86::Xmm& dd_bd = f->dd_bd;
...
// ============================================================================
// [b2d::Pipe2D::FetchGradientConicalPart - Construction / Destruction]
// ============================================================================

FetchGradientConicalPart::FetchGradientConicalPart(PipeCompiler* pc, uint32_t fetchType, uint32_t fetchExtra, uint32_t format) noexcept
  : FetchGradientPart(pc, fetchType, fetchExtra, format) {
...
// ============================================================================
// [b2d::Pipe2D::FetchGradientConicalPart - Init / Fini]
// ============================================================================

void FetchGradientConicalPart::_initPart(x86::Gp& x, x86::Gp& y) noexcept {
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  f->table          = cc->newIntPtr("f.table");       // Reg.
  f->xx_xy          = cc->newXmmPd("f.xx_xy");        // Mem.
  f->yx_yy          = cc->newXmmPd("f.yx_yy");        // Mem.
  ...
  if (maxPixels() > 1) {
    ...
    pc->vmulpd(f->xx4_xy4, f->xx_xy, pc->constAsMem(gConstants.xmm_d_4));
    cc->spill(f->xx4_xy4);

    pc->vswizi32(f->xx_0123, f->xy_0123, x86::Predicate::shuf(0, 0, 0, 0));
    pc->vswizi32(f->xy_0123, f->xy_0123, x86::Predicate::shuf(1, 1, 1, 1));

    pc->vmulps(f->xx_0123, f->xx_0123, pc->constAsMem(gConstants.xmm_f_0123));
    pc->vmulps(f->xy_0123, f->xy_0123, pc->constAsMem(gConstants.xmm_f_0123));
...
void FetchGradientConicalPart::_finiPart() noexcept {}

// ============================================================================
// [b2d::Pipe2D::FetchGradientConicalPart - Advance]
// ============================================================================

void FetchGradientConicalPart::advanceY() noexcept {
  pc->vaddpd(f->hx_hy, f->hx_hy, f->yx_yy);
}

void FetchGradientConicalPart::startAtX(x86::Gp& x) noexcept {
  if (isRectFill()) {
...
void FetchGradientConicalPart::advanceX(x86::Gp& x, x86::Gp& diff) noexcept {
  B2D_UNUSED(diff);
...
// ============================================================================
// [b2d::Pipe2D::FetchGradientConicalPart - Fetch]
// ============================================================================

void FetchGradientConicalPart::fetch1(PixelARGB& p, uint32_t flags) noexcept {
  ...
  pc->vmovaps(x1, pc->constAsMem(gConstants.xmm_f_abs));
  pc->vmovaps(x2, pc->constAsMem(gConstants.xmm_f_1e_m20));
  ...
  pc->vsrai32(x0, x0, 31);
  pc->vandps(x1, x1, x86::ptr(consts, REL(Constants::Conical, n_div_4)));

  pc->vmulss(x2, x3, x3);
  pc->vandps(x0, x0, x86::ptr(consts, REL(Constants::Conical, n_extra)));

  pc->vmulss(x4, x2, x86::ptr(consts, REL(Constants::Conical, q3)));
  pc->vaddss(x4, x4, x86::ptr(consts, REL(Constants::Conical, q2)));

  pc->vmulss(x4, x4, x2);
  pc->vaddss(x4, x4, x86::ptr(consts, REL(Constants::Conical, q1)));

  pc->vmulss(x2, x2, x4);
  pc->vaddss(x2, x2, x86::ptr(consts, REL(Constants::Conical, q0)));

  pc->vmulss(x2, x2, x3);
  pc->vsubss(x2, x2, x1);

  pc->vswizps(x1, x0, x86::Predicate::shuf(2, 3, 0, 1));
  pc->vandps(x2, x2, pc->constAsMem(gConstants.xmm_f_abs));
...
void FetchGradientConicalPart::prefetchN() noexcept {
  x86::Gp& consts = f->consts;
  x86::Xmm& px_py = f->px_py;
  x86::Xmm& x0 = f->x0;
  x86::Xmm& x1 = f->x1;
  ...
  pc->vmovaps(x2, pc->constAsMem(gConstants.xmm_f_abs));
  ...
  pc->vmovaps(x4, pc->constAsMem(gConstants.xmm_f_1e_m20));
  pc->vandps(x3, x2, x1);
  pc->vandps(x2, x2, x0);
  ...
  pc->vsrai32(x0, x0, 31);
  pc->vandps(x2, x2, x86::ptr(consts, REL(Constants::Conical, n_div_4)));

  pc->vsrai32(x1, x1, 31);
  pc->vandps(x0, x0, x86::ptr(consts, REL(Constants::Conical, n_div_2)));

  pc->vmulps(x5, x3, x3);
  pc->vandps(x1, x1, x86::ptr(consts, REL(Constants::Conical, n_div_1)));

  pc->vmulps(x4, x5, x86::ptr(consts, REL(Constants::Conical, q3)));
  pc->vaddps(x4, x4, x86::ptr(consts, REL(Constants::Conical, q2)));

  pc->vmulps(x4, x4, x5);
  pc->vaddps(x4, x4, x86::ptr(consts, REL(Constants::Conical, q1)));

  pc->vmulps(x5, x5, x4);
  pc->vaddps(x5, x5, x86::ptr(consts, REL(Constants::Conical, q0)));

  pc->vmulps(x5, x5, x3);
  pc->vsubps(x5, x5, x2);

  pc->vandps(x5, x5, pc->constAsMem(gConstants.xmm_f_abs));
...
void FetchGradientConicalPart::fetch4(PixelARGB& p, uint32_t flags) noexcept {
  x86::Gp& consts = f->consts;
  x86::Xmm& px_py = f->px_py;
  x86::Xmm& x0 = f->x0;
  x86::Xmm& x1 = f->x1;
  ...
  pc->vmovaps(x5, x86::ptr(consts, REL(Constants::Conical, q3)));
  pc->vmulps(x5, x5, x4);
  pc->vandps(x1, x1, x86::ptr(consts, REL(Constants::Conical, n_div_1)));
  pc->vaddps(x5, x5, x86::ptr(consts, REL(Constants::Conical, q2)));
  pc->vmulps(x5, x5, x4);
  fCtx.fetchARGB32(x86::dword_ptr(f->table, idx0, 2));

  pc->vaddps(x5, x5, x86::ptr(consts, REL(Constants::Conical, q1)));
  pc->vmulps(x5, x5, x4);
  pc->vaddps(x5, x5, x86::ptr(consts, REL(Constants::Conical, q0)));
  pc->vmulps(x5, x5, x3);
  fCtx.fetchARGB32(x86::dword_ptr(f->table, idx1, 2));
