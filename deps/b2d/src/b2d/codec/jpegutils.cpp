...
// ============================================================================
// [b2d::JpegInternal - IDCT (Ref)]
// ============================================================================

#define B2D_JPEG_IDCT_IDCT(s0, s1, s2, s3, s4, s5, s6, s7) \
  int x0, x1, x2, x3;                             \
  int t0, t1, t2, t3;                             \
  int p1, p2, p3, p4, p5;                         \
                                                  \
  p2 = s2;                                        \
  p3 = s6;                                        \
  p1 = (p2 + p3) * B2D_JPEG_IDCT_P_0_541196100;   \
  t2 = p3 * B2D_JPEG_IDCT_M_1_847759065 + p1;     \
  t3 = p2 * B2D_JPEG_IDCT_P_0_765366865 + p1;     \
                                                  \
  p2 = s0;                                        \
  p3 = s4;                                        \
  t0 = B2D_JPEG_IDCT_SCALE(p2 + p3);              \
  t1 = B2D_JPEG_IDCT_SCALE(p2 - p3);              \
                                                  \
  x0 = t0 + t3;                                   \
  x3 = t0 - t3;                                   \
  x1 = t1 + t2;                                   \
  ...
  p5 = p3 + p4;                                   \
                                                  \
  p5 = p5 * B2D_JPEG_IDCT_P_1_175875602;          \
  t0 = t0 * B2D_JPEG_IDCT_P_0_298631336;          \
  t1 = t1 * B2D_JPEG_IDCT_P_2_053119869;          \
  t2 = t2 * B2D_JPEG_IDCT_P_3_072711026;          \
  t3 = t3 * B2D_JPEG_IDCT_P_1_501321110;          \
                                                  \
  p1 = p1 * B2D_JPEG_IDCT_M_0_899976223 + p5;     \
  p2 = p2 * B2D_JPEG_IDCT_M_2_562915447 + p5;     \
  p3 = p3 * B2D_JPEG_IDCT_M_1_961570560;          \
  p4 = p4 * B2D_JPEG_IDCT_M_0_390180644;          \
                                                  \
  t3 += p1 + p4;                                  \
  t2 += p2 + p3;                                  \
  t1 += p2 + p4;                                  \
...
void B2D_CDECL idct8_ref(uint8_t* dst, intptr_t dstStride, const int16_t* src, const uint16_t* qTable) noexcept {
  ...
  for (i = 0, tmp = tmpData; i < 8; i++, src++, tmp++, qTable++) {
    ...
    if (src[8] == 0 && src[16] == 0 && src[24] == 0 && src[32] == 0 && src[40] == 0 && src[48] == 0 && src[56] == 0) {
      int32_t dcTerm = (int32_t(src[0]) * int32_t(qTable[0])) << (B2D_JPEG_IDCT_PREC - B2D_JPEG_IDCT_COL_NORM);
      ...
      x0 += B2D_JPEG_IDCT_COL_BIAS;
      x1 += B2D_JPEG_IDCT_COL_BIAS;
      x2 += B2D_JPEG_IDCT_COL_BIAS;
      x3 += B2D_JPEG_IDCT_COL_BIAS;

      tmp[ 0] = (x0 + t3) >> B2D_JPEG_IDCT_COL_NORM;
      tmp[56] = (x0 - t3) >> B2D_JPEG_IDCT_COL_NORM;
      tmp[ 8] = (x1 + t2) >> B2D_JPEG_IDCT_COL_NORM;
      tmp[48] = (x1 - t2) >> B2D_JPEG_IDCT_COL_NORM;
      tmp[16] = (x2 + t1) >> B2D_JPEG_IDCT_COL_NORM;
      tmp[40] = (x2 - t1) >> B2D_JPEG_IDCT_COL_NORM;
      tmp[24] = (x3 + t0) >> B2D_JPEG_IDCT_COL_NORM;
      tmp[32] = (x3 - t0) >> B2D_JPEG_IDCT_COL_NORM;
    }
  }

  for (i = 0, tmp = tmpData; i < 8; i++, dst += dstStride, tmp += 8) {
    B2D_JPEG_IDCT_IDCT(tmp[0], tmp[1], tmp[2], tmp[3], tmp[4], tmp[5], tmp[6], tmp[7])

    x0 += B2D_JPEG_IDCT_ROW_BIAS;
    x1 += B2D_JPEG_IDCT_ROW_BIAS;
    x2 += B2D_JPEG_IDCT_ROW_BIAS;
    x3 += B2D_JPEG_IDCT_ROW_BIAS;

    dst[0] = Support::clampToByte((x0 + t3) >> B2D_JPEG_IDCT_ROW_NORM);
    dst[7] = Support::clampToByte((x0 - t3) >> B2D_JPEG_IDCT_ROW_NORM);
    dst[1] = Support::clampToByte((x1 + t2) >> B2D_JPEG_IDCT_ROW_NORM);
    dst[6] = Support::clampToByte((x1 - t2) >> B2D_JPEG_IDCT_ROW_NORM);
    dst[2] = Support::clampToByte((x2 + t1) >> B2D_JPEG_IDCT_ROW_NORM);
    dst[5] = Support::clampToByte((x2 - t1) >> B2D_JPEG_IDCT_ROW_NORM);
    dst[3] = Support::clampToByte((x3 + t0) >> B2D_JPEG_IDCT_ROW_NORM);
    dst[4] = Support::clampToByte((x3 - t0) >> B2D_JPEG_IDCT_ROW_NORM);
  }
}

// ============================================================================
// [b2d::JpegInternal - Upsample (Ref)]
// ============================================================================
...
// ============================================================================
// [b2d::JpegInternal - ConvYCbCrToRGB32 (Ref)]
// ============================================================================
...
void B2D_CDECL convYCbCr8ToRGB32_ref(uint8_t* dst, const uint8_t* pY, const uint8_t* pCb, const uint8_t* pCr, uint32_t count) noexcept {
  for (uint32_t i = 0; i < count; i++) {
    int yy = (int(pY[i]) << B2D_JPEG_YCBCR_PREC) + (1 << (B2D_JPEG_YCBCR_PREC - 1));
